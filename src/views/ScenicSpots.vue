...
export default {
  ...
  setup() {
    ...
    function getCitySpots() {
      axios({
        method: "get",
        url: `https://ptx.transportdata.tw/MOTC/v2/Tourism/ScenicSpot/${tempCity.value}?$format=JSON`,
        headers: getAuthorizationHeader(),
      })
        .then((res) => {
          scenicSpots.value = res.data;
    ...
    function filterSenicSpots(page = 1) {
      tempScenicSpots.value = scenicSpots.value;
      if (tempCategory.value.value) {
        tempScenicSpots.value = tempScenicSpots.value.filter((item) => {
          return (
      ...
      //     });
      //   }
      pagination.value.totalResult = tempScenicSpots.value.length;
      ...
      tempScenicSpots.value = tempScenicSpots.value.slice(
        firstItem,
        lastItem + 1
      );
    }
        ...
        <template v-if="tempScenicSpots.length">
          <div class="row gy-3 mb-6">
            ...
            >
              <GeneralCard
                :item="scenicSpot"
                cardPath="scenicspots"
              ></GeneralCard>
            </div>
          </div>
          <Pagination
            :pages="pagination"
            @update-page="filterSenicSpots"
          ></Pagination>
        </template>
      </section>
    </div>
